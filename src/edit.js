...
function updateBlocks() { for (var a = 1; a <= $('.block').length; a++) { $('.block:nth-of-type('+a+')').prop('id', 'block_'+a).children('span:nth-of-type(2)').text(a); $('.block:nth-of-type('+a+')').find('div.code').prop('id', 'code_'+a); } }
$(function() {
  browser.storage.local.get().then(function(item) { 
    if (item.styling_1) {
      ...
      for (var e = 1; e <= blocks; e++) {
        ...
        if (item.styling_1["block_"+e].code) { $('body .block:nth-of-type('+e+') textarea.code').val(item.styling_1["block_"+e].code); updateBlocks(); }
    ...
    for (var a = 1; a <= $('.block').length; a++) { ace.edit('code_'+a, { mode: "ace/mode/css", selectionStyle: "text" }); }
  ...
  $(document).on('input', 'textarea', function() { updateBlocks(); });
  ...
  $(document).on('click', '.add_block', function() { $(this).parent().clone().find('textarea.code').val('').end().find('input').val('').end().find('section:not(:first-of-type)').remove().end().prop('id', '').insertAfter($(this).parent()); updateBlocks(); });
