...
function updateTextarea() {
  var lines = document.querySelector("textarea").value.split(/\r*\n/).length;
...
$(function() {
  ...
  updateTextarea();
  $('#save').click(function() {
    browser.storage.local.set({ styling_1: { block_1: { code_1: $('textarea.code').val().replace(/^|\s+$/g, ''), url_1: $('input.url').val(), url_1_type: $('select').val() } } }).then(onChange, onError); 
  ...
  $('textarea').bind('input propertychange', function() { updateTextarea(); }).resize(function() { $('.side').css('height', $('textarea').css('height')); });
