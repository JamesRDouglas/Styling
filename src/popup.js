...
function getSubdomain(url) { url = url.replace(/(https?:\/\/)?(www.)?/i, ''); if (url.indexOf('/') !== -1) { return url.split('/')[0]; } return url; }
...
$('#url').prop('href', browser.extension.getURL("src/manage.html?action=create&type=url&target="+window.location.href));
$('#domain').prop('href', browser.extension.getURL("src/manage.html?action=create&type=domain&target="+window.hostname));
$('#subdomain').prop('href', browser.extension.getURL("src/manage.html?action=create&type=domain&target="+getSubdomain(window.location.href)));
$(function() {
