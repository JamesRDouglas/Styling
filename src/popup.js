...
function generateNewStyle(type, url) { var newStyleId; browser.storage.local.get(function(item) { var currentStyle = item.default; if (item.styles.length === 0) { newStyleId = 1; } else { newStyleId = item.styles.length+1; } currentStyle.id = newStyleId; currentStyle.blocks[0].urls[0].type = type; currentStyle.blocks[0].urls[0].address = url; item.styles.push(currentStyle); browser.storage.local.set(item).then(function() { browser.tabs.create({ url: browser.extension.getURL('src/edit.html?style='+newStyleId) }); window.close(); }, onError); }); }
        ...
        for (var b = 0; b < item.styles[s].blocks.length; b++) {
          ...
          for (var u = 0; u < urls; u++) {
            if (item.styles[s].blocks[0].urls[u]) {
              ...
              ) { applicable_styles.push({ id: item.styles[s].id, name: item.styles[s].name, is_enabled: item.styles[s].enabled }); break block; }
            }
          }
        }
      }
