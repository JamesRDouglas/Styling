...
$(function() {
  ...
  browser.storage.local.get().then(function(item) {
    ...
    if (typeof style_id === "undefined" || style_id <= 0) { window.location.href = 'edit.html?style=1'; }
    for (a = 0; a < item.styles.length; a++) { styles_arr.push(item.styles[a].id); }
    style_id = styles_arr.indexOf(style_id);
    if (typeof item.styles[style_id].options === "undefined") { item.styles[style_id].options = default_style.options; browser.storage.local.set(item).catch(onError); }
    ...
    if (item.styles[style_id].options.error_marker.enabled === true) { $('#error-marker').prop('checked', true); }
    ...
    updateBlocks(false);
    $('.code').each(function() { var editor = ace.edit(this); editor.getSession().on('change', function() { checkForChanges(style_id); }); });
  ...
  $('#enabled').click(function() { browser.storage.local.get(function(item) { if ($('#enabled').is(':checked')) { item.styles[style_id].disabled = false; browser.storage.local.set(item).catch(onError); browser.tabs.query({ currentWindow: true }).then(function(tabs) { sendMessageToTabs(tabs,"update"); }).catch(onError); } else { item.styles[style_id].disabled = true; browser.storage.local.set(item).catch(onError); browser.tabs.query({ currentWindow: true }).then(function(tabs) { sendMessageToTabs(tabs,"update"); }).catch(onError); } }); });
...
browser.runtime.onMessage.addListener(function(message) { if (message.action === "update") { browser.storage.local.get().then(function(item) { var style_id = new URLSearchParams(window.location.search).get('style'); if (item.styles[style_id].disabled === true) { $('#enabled').prop('checked', false); } else { $('#enabled').prop('checked', true); } }); } });
