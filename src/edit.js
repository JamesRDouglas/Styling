...
function updateErrors() { $('#errors').empty(); for (e = 0; e < editors.length; e++) { $('#errors').append('<div id="errors-'+(e+1)+'" class="block-container unselectable"><div class="error-block"><span>Block '+(e+1)+'</span></div><div class="error-container"></div></div>'); var annot = editors[e].getSession().getAnnotations(); for (a = 0; a < annot.length; a++) { if (annot[a].type === 'error') { imageData = errorImage; } else if (annot[a].type === 'warning') { imageData = warningImage; } else if (annot[a].type === 'info') { break; } $('div.block-container#errors-'+(e+1)+' div.error-container').append('<small title="'+annot[a].text+'"><i style="background-image: '+imageData+';"></i> — Line: '+(annot[a].row+1)+', Col:  '+(annot[a].column+1)+'<br><span>'+annot[a].text+'</span></small>'); } if ($('div.block-container#errors-'+(e+1)+' div.error-container').is(':empty')) { $('div.block-container#errors-'+(e+1)).remove(); } } if ($('#errors').is(':empty')) { $('#errors-container').hide(); } else if ($('#errors-container').css('display')==='none') { $('#errors-container').fadeIn(); } }
