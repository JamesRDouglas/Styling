...
$(function() {
  numOfLines(document.querySelector("textarea"), toString($('textarea').css('line-height')).slice(0, -2));
  browser.storage.local.get().then(function(item) { 
    if (item.styling_1) {
      ...
      if (item.styling_1.block_1.url_1_type == 'everything') { $('input').hide(); }
  ...
  $('select').change(function() { if ($(this).val() == "everything") { $('input').hide(); } else { $('input').show(); } });
});