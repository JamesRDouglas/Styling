...
function updateErrors() { $('#errors').empty(); for (e = 0; e < editors.length; e++) { $('#errors').append('<div class="block-container unselectable"><div class="error-block"><span>Block '+(e+1)+'</span></div><div class="error-container"></div></div>'); var annot = editors[e].getSession().getAnnotations(); for (a = 0; a < annot.length; a++) { if (annot[a].type === 'error') { imageData = errorImage; } else if (annot[a].type === 'warning') { imageData = warningImage; } else if (annot[a].type === 'info') { break; } $('div.block-container:nth-of-type('+(e+1)+') div.error-container').append("<small><i style='background-image: "+imageData+";'></i> — Line: "+(annot[a].row+1)+", Col:  "+(annot[a].column+1)+"<br><span>"+annot[a].text+"</span></small>"); } if ($('div.block-container:nth-of-type('+(e+1)+') div.error-container > small').length) { $('div.block-container:nth-of-type('+(e+1)+')').css('display', 'block'); } } if ($('#errors').is(':empty')) { $('#errors').append('<i>No errors detected</i>'); } }
...
$(function() {
  ...
  $(document).on('change paste keyup', '.url, select:not(.options)', function() { checkForChanges(style_index); });
